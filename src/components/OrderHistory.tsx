  ...
  return (
    <main style={{ minHeight: '100vh', padding: '24px 16px 120px', background: '#f7f4f2', color: '#212121' }}>
      <div style={{ maxWidth: '760px', margin: '0 auto' }}>
        ...
        {!signedIn ? <section style={{ padding: '45px 24px', border: '1px solid #e7dfda', borderRadius: '20px', background: '#fff', textAlign: 'center' }}><ShoppingBag size={40} color="var(--accent-red)" /><h2>Sign in with WhatsApp to see your orders</h2><p style={{ color: '#777', fontSize: '13px' }}>Verify your WhatsApp number to view past orders and live tracking.</p><Link href="/account/login" style={{ display: 'inline-block', marginTop: '8px', padding: '12px 18px', borderRadius: '10px', background: '#25d366', color: '#fff', textDecoration: 'none', fontWeight: 800 }}>Login with WhatsApp</Link></section> : orders.length === 0 ? <section style={{ padding: '45px 24px', border: '1px solid #e7dfda', borderRadius: '20px', background: '#fff', textAlign: 'center' }}><ShoppingBag size={40} color="var(--accent-red)" /><h2>No orders yet</h2><p style={{ color: '#777', fontSize: '13px' }}>Your first online order will appear here.</p></section> : (
          <div style={{ display: 'grid', gap: '14px' }}>{orders.map((order) => <article key={order.id} style={{ padding: '20px', border: '1px solid #e7dfda', borderRadius: '18px', background: '#fff', boxShadow: '0 8px 25px rgba(33,33,33,.04)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
              <div>
                ...
                <span style={{ display: 'flex', marginTop: '5px', alignItems: 'center', gap: '5px', color: '#888', fontSize: '10px' }}><Clock3 size={12} /> {orderDateFormatter.format(new Date(order.createdAt))}</span>
              </div>
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                {order.paymentStatus && (
                  <span style={{ padding: '5px 8px', borderRadius: '999px', background: order.paymentStatus === 'PAID' ? '#dcfce7' : (order.paymentStatus === 'FAILED' ? '#fee2e2' : '#fef3c7'), color: order.paymentStatus === 'PAID' ? '#15803d' : (order.paymentStatus === 'FAILED' ? '#b91c1c' : '#b45309'), fontSize: '9px', fontWeight: 900, textTransform: 'uppercase' }}>
                    {order.paymentStatus}
                  </span>
                )}
