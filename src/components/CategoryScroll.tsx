...
export default function CategoryScroll() {
  ...
  return (
    <div style={{ display: 'flex', overflowX: 'auto', padding: '16px', gap: '16px', background: 'var(--bg-dark)', position: 'sticky', top: 0, zIndex: 40 }} className="hide-scrollbar">
      ...
      {categories.map((cat) => {
        ...
        return (
          ...
          >
            <div className="cat-img" style={{ width: '60px', height: '60px', borderRadius: '50%', overflow: 'hidden', border: isActive ? '2px solid var(--accent-gold)' : '2px solid transparent' }}>
              <Image src={cat.image} alt={cat.name} width={60} height={60} style={{ objectFit: 'cover' }} />
            </div>
            <span style={{ fontSize: '12px', fontWeight: isActive ? 'bold' : 'normal', borderBottom: isActive ? '2px solid var(--accent-gold)' : 'none', paddingBottom: '4px' }}>{cat.name}</span>
          </Link>
        );
      })}
    </div>
